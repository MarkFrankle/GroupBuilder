...
async def get_organization_details(
    ...
    for member_doc in member_docs:
        ...
        joined_at_unix = joined_at.timestamp() if joined_at else datetime.now(timezone.utc).timestamp()
    ...
    for invite_doc in invite_docs:
        ...
        created_at_unix = created_at.timestamp() if created_at else datetime.now(timezone.utc).timestamp()
        
        expires_at = invite_data.get("expires_at")
        expires_at_unix = expires_at.timestamp() if expires_at else datetime.now(timezone.utc).timestamp()
...
async def revoke_invite(
    ...
    # Get invite
    invite_ref = db.collection("organizations").document(org_id).collection("invites").document(invite_id)
...
async def remove_member(
    ...
    if member_email:
        org_ref = db.collection("organizations").document(org_id)
        invites_query = org_ref.collection("invites").where("email", "==", member_email.lower()).where("status", "==", "accepted").limit(1)
