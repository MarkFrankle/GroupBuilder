...
from dataframe_to_participant_dict import dataframe_to_participant_dict
import pandas as pd

class GroupBuilder:
    def __init__(self, participants, num_tables, num_sessions):
        self.participants = dataframe_to_participant_dict(participants)
    ...
    def _add_participant_attribute_distribution_constraint(self, attribute_name, attribute_set) -> None:
        ...
        for s in self.sessions:
            for attribute_value in attribute_set:
                max_participants_per_attribute[(s,attribute_value)] = self.model.NewIntVar(0, len(self.participants),
                    f'max_participants_{attribute_name}_s{s}_{attribute_name[0]}{attribute_value}')
                min_participants_per_attribute[(s,attribute_value)] = self.model.NewIntVar(0, len(self.participants),
                    f'min_participants_{attribute_name}_s{s}_{attribute_name[0]}{attribute_value}')

                for t in self.tables:
                    table_participant_count_per_attribute = sum(
                        self.x[(p['id'], s, t)] for p in self.participants if p[attribute_name] == attribute_value
                    )
