...
def handle_generate_assignments(
    ...
    if use_incremental:
        logger.info(
            f"Using incremental solver (batch size: 2, timeout: {max_time_seconds}s)"
        )
        results = builder.generate_assignments_incremental(
            batch_size=2, max_time_seconds=max_time_seconds
        )
    else:
